...
Only contests that extracted unambiguously from the PDF text layer are
included. The Democratic Central Committee and Board of Education races on
this ballot style use a two-column layout that pdftotext interleaves
unreliably; they are intentionally left out rather than risk mis-mapping a
candidate to the wrong contest. A follow-up pass with a column-aware parser
(e.g. pdfplumber word coordinates) is needed to add those safely.
...
# (office_name, office_level, contest_label, district_layer_id, district_id, scope, vote_for, order, [(name, unopposed), ...])
...
CONTESTS = [
    ("Governor / Lieutenant Governor", "State", "Governor / Lieutenant Governor", None, None, "at_large", 1, 1, [
        ("Eric S. Felber / LaTrece Hawkins Lytes", False),
        ("Wes Moore / Aruna Miller", False),
    ]),
    ("Comptroller", "State", "Comptroller", None, None, "at_large", 1, 2, [
        ("Brooke Elizabeth Lierman", True),
    ]),
    ("Attorney General", "State", "Attorney General", None, None, "at_large", 1, 3, [
        ("Anthony G. Brown", True),
    ]),
    ("U.S. Representative", "Federal", "Representative in Congress District 8", "cd119", "2408", "district", 1, 4, [
        ("J. D. Kumar", False),
        ("Stephen Alan Leon", False),
        ("Jamie Raskin", False),
        ("Boris Kabel Velasquez", False),
    ]),
    ("State Senator", "State", "State Senator District 14", "sldu", "24014", "district", 1, 5, [
        ("Craig J. Zucker", True),
    ]),
    ("House of Delegates", "State", "House of Delegates District 14", "md_delegate_subdistricts_2022", "14", "district", 3, 6, [
        ("Alicia Contreras-Donello", False),
        ("Anne R. Kaiser", False),
        ("Bernice Mireku-North", False),
        ("Matt Post", False),
    ]),
    ("County Executive", "Local", "County Executive", "county", "24031", "at_large", 1, 7, [
    ...
    ("County Council", "Local", "County Council At-Large", "county", "24031", "at_large", 4, 8, [
    ...
    ("County Council", "Local", "County Council District 7", None, None, "district", 1, 9, [
        ("Van Free", False),
        ("Sharif Hidayat", False),
        ("Dawn Luedtke", False),
    ]),
    ("Judge of the Circuit Court", "Local", "Judge of the Circuit Court Circuit 6", None, None, "at_large", 4, 10, [
    ...
    ("State's Attorney", "Local", "State's Attorney", "county", "24031", "at_large", 1, 11, [
        ("John McCarthy", True),
    ]),
    ("Clerk of the Circuit Court", "Local", "Clerk of the Circuit Court", "county", "24031", "at_large", 1, 12, [
        ("Karen Bushell", True),
    ]),
    ("Register of Wills", "Local", "Register of Wills", "county", "24031", "at_large", 1, 13, [
        ("Alan S. Bowser", False),
        ("Paul J. Dollahite", False),
        ("Barbara Ebel", False),
    ]),
    ("Sheriff", "Local", "Sheriff", "county", "24031", "at_large", 1, 14, [
        ("Will Milam", False),
...
def main():
    ...
    for office_name, level, label, layer_id, district_id, scope, vote_for, order, candidates in CONTESTS:
        out.write(
        ...
        out.write(
            ...
            "scope, vote_for, order_on_ballot, source_id)\n"
            "select '22222222-2222-2222-2222-222222222222', o.office_id, "
            f"{sql_text(layer_id)}, {sql_text(district_id)}, {sql_text(label)}, {sql_text(scope)}, {vote_for}, {order}, "
            ...
            "  set vote_for = excluded.vote_for, scope = excluded.scope;\n\n"
        )
        ...
        for i, (cand_name, unopposed) in enumerate(candidates, start=1):
            out.write(
                ...
                "select c.contest_id, "
                f"{sql_text(cand_name)}, {sql_text('Democratic')}, {sql_bool(unopposed)}, {i}, "
                "'11111111-1111-1111-1111-111111111111'\n"
                "from contests c\n"
